...
A fourth rotation slot alongside TIME/TEMPERATURE/HUMIDITY: full-face
240×240 images (`main/photo_face_*_data.cpp`, round-robin via
`setPhotos()`) with a one-pass CRT-style scan-bar sweep and flicker,
wired in via `DispDriverGc9a01RoundClock`. The driver itself has no
opinion on the images' content — same generic seam as
`IDisplayTimeProvider` — so swapping in a different photo is just a
matter of running `scripts/convert_photo.py path/to/image.jpg
var_name photo_face.h` against a new square source image and
rebuilding. Most of the rotation is Google Gemini-generated portraits;
the "test signal" slot (`photo_face_test_signal`) is instead a
code-synthesized art-deco TV test card (PIL, see
`scripts/gen_deco_test_card.py`), not AI-generated art.
